...
AccessToken은 전역 상태(Zustand 메모리)에 저장하며, API 호출 시 Axios Interceptor를 통해 자동으로 Bearer 헤더에 첨부되어 전송됩니다. RefreshToken은 httpOnly Cookie로 안전하게 보관되며, AccessToken 만료 시 Silent Refresh에 사용됩니다.
